...
lookup' key (BNode left nodeKey nodeValue right)
      | key == nodeKey = nodeValue
      | key < nodeKey = lookup' key left
      | key > nodeKey = lookup' key right
...
insert key val (BNode left nodeKey nodeValue right)
  ...
  | key < nodeKey  = BNode (insert key val left) nodeKey nodeValue right 
  | key > nodeKey  = BNode left nodeKey nodeValue (insert key val right) 


delete :: Int -> IntSearchTree value -> IntSearchTree value
delete _ Empty = Empty  
delete key (BNode left nodeKey nodeValue right)
  | key == nodeKey = BNode left nodeKey Nothing right  
  | key < nodeKey  = BNode (delete key left) nodeKey nodeValue right 
  | key > nodeKey  = BNode left nodeKey nodeValue (delete key right) 
...
printTree Empty = "" 
printTree (BNode left key Nothing right) =
    "(" ++ printTree left ++ ") " ++ show key ++ " (" ++ printTree right ++ ")"
printTree (BNode left key (Just value) right) =
    "(" ++ printTree left ++ ") " ++ show key ++ " (" ++ printTree right ++ ")"
